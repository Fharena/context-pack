...
5. 생성 pack에 가장 작은 시작 범위와 선택 이유를 표시합니다.
...
benchmark harness는 deterministic orientation을 측정합니다. selected area, first-read text budget, latency, clone 후 handoff replay를 확인합니다. 자세한 내용은 [docs/BENCHMARKS.ko.md](docs/BENCHMARKS.ko.md)와 [docs/benchmarks/latest.md](docs/benchmarks/latest.md)를 보세요.

현재 공개 저장소 10개 검증은 미리 정한 routing 기준을 모두 통과했습니다. 예를 들면 `psf/requests`의 예상 first-read ratio는 11%, `mozilla/BrowserQuest`는 16%였고, fresh clone handoff replay는 12% 범위에서 같은 routing signature를 재현했습니다.

token 수치는 provider billing token이 아니라 대략적인 `chars/4` text-budget 추정입니다. 독립 에이전트가 더 좋은 patch를 만든다는 증거도 아닙니다. patch quality A/B는 별도 검증 과제입니다.
