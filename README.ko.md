...
CI는 이 작은 repo 흐름을 그대로 검증합니다. 버그 작업과 failing-test prompt는 `source, tests`에서 시작하고, branch review는 committed diff를 읽으며, `checkpoint --pack`은 tracked 파일을 더럽히지 않고 ignored local handoff state만 남깁니다.
